\section{A uniform heat estimate on \(\USp(2g)\)}
...
\tag{A.1}\label{eq:heat-generator}
\]
...
\end{split}
\tag{A.4}\label{eq:weyl-dimension}
\]
...
\[
 ...
 \le \exp\!\bigl(Cg^2\log(g+2)\bigr)
       s^{-(2g^2+g+1)/2}.}
\tag{A.5}\label{eq:uniform-heat-trace}
\]
...
\tag{A.6}\label{eq:dimension-crude}
\]
...
\tag{A.8}\label{eq:brownian-distance}
\]
\end{lemma}
...
\tag{A.9}\label{eq:abstract-character-bound}
\]
...
\tag{A.10}\label{eq:abstract-heat-bridge}
\]
\end{proposition}
