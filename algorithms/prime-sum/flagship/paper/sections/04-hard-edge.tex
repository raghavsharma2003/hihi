...
projection, so the determinantal process exists by Soshnikov's Theorem~3
\cite{Soshnikov2000}.  His Theorem~5 gives convergence of all local
count-cylinder laws.  The uniform first-moment estimate
\(\E\Xi_g([0,L])\le2L\) gives tightness of the restrictions to every compact
interval; a diagonal compact-exhaustion argument therefore upgrades the
cylinder-law convergence to weak convergence for the vague topology:
\begin{equation}
...
\end{equation}
Indeed, writing \((n)_N=n(n-1)\cdots(n-N+1)\), the factorial-moment formula
\cite[Definitions~2 and~3, equations (1.5) and (1.20)]{Soshnikov2000} is
\[
...
\(\prod_{j=1}^N J_0(a_jt)\), where \(J_0\) is the Bessel function of the first
kind of order zero.  Uniformly for \(a_j\in[a_0,4]\) and
