...
Fiorilli--Martin density asymptotics and an extension of their aggregate
quadratic race from $m=0$ to growing weights, proved here with an error term
...
constants), while the critical corner $M\log q\to\lambda$ is governed
by the microscopic scaled-zero point measure: under its natural vague
convergence and an $L^2$-tail condition we prove convergence to an
explicit, generally non-Gaussian Bessel-product law; and
$m\mapsto\delta_\chi(m)$ is \emph{strictly
...
inside one statement. Section~\ref{sec:critical} then classifies the
excluded critical scaling $M\log q\to\lambda$: its limiting law is a
functional of the microscopic low-zero point process, not a universal
Gaussian. Section~\ref{sec:monotone-large} proves
...
The third-order formula of Theorem~\ref{thm:joint}(iii), evaluated by
the same independent script, reduces the relative residuals in the
$(4,8)$, $(4,20)$ and $(3,60)$ rows respectively to
$-2.04\times10^{-4}$, $-6.18\times10^{-6}$ and
$+7.74\times10^{-7}$.  At $(4,100)$ it gives $+2.14\times10^{-6}$;
both this and the second-order residual are below the
$9.9\times10^{-6}$ modelling floor, so their ordering has no numerical
significance.  These figures are checks, not inputs to the proof.
...
\]
Taylor's formula for $e^E$ and for $\sin t/t$, with the surviving
Gaussian factor $e^{-(8/17)\sigma^2t^2}$ from
\eqref{eq:Eunif}, now gives
\[
\begin{split}
e^{E(t)}\frac{\sin t}{t}
...
\end{split}
\]
After multiplication by $e^{-\sigma^2t^2/2}$ and integration, the
remainder is $O(G\sigma^{-6})$.  For completeness, every omitted term
is bounded by an absolute constant times the surviving Gaussian times
a sum of
\[
 t^6,quad S_4t^8,quad S_6t^8,quad S_8t^8,quad
 S_4^2t^{10},\quad S_4S_6t^{10},\quad
 S_4^3t^{12},\quad S_6^2t^{12};
\]
the moment identity in \eqref{eq:decomp}, the three displayed moment
bounds, and $\sigma\ge1$ make each integral $O(G\sigma^{-6})$.
Taylor remainders of still higher degree are absorbed by the same
Gaussian using $|E(t)|\le\sigma^2t^2/34$.  Extending the seven retained
moments to infinity, as in part (ii), has exponentially small cost.
...
There is a normalization slip in the two displays of \cite[\S3.6]{FM13}
that does not affect their surrounding low-zero discussion. With their
definition
...
\]
not the mean-$2$ variable printed there; consequently the leading
correction is $(2\pi b(\chi))^{-1/2}$, rather than twice that quantity.
This also follows by taking $m=0$ in
...
zero.  We now identify what replaces it at the transition scale
$M\asymp1/\log q$.  The answer is not another Gaussian formula: the
limiting law is a continuous functional of the microscopic zero
configuration.  This makes precise the low-zero mechanism visible at
...
\end{equation}
Because $(\lambda_n)$ is bounded, the squared right-hand side is at
most $C_\lambda y^{-2}$ for $y\ge1$.  Vague convergence controls every
compact interval, while \eqref{eq:critical-tail} controls the
complement.  It follows first that
$\int b_\lambda^2\,d\mu<\infty$, and hence the defining series for
$\mathcal X_{\lambda,\mu}$ converges a.s.\ and in $L^2$ by the same
independent mean-zero series argument used for $X_{m,\chi}$.

We prove convergence through characteristic functions.  Fix $t$ and
choose $R$ with $\mu(\{R\})=0$.  On $[0,R]$, vague convergence of
integer-valued locally finite point measures implies convergence of
the finite configurations, with multiplicity: enclose the finitely
many limiting atoms in disjoint intervals whose boundaries carry no
mass and use convergence of their integer masses.  Together with
$\lambda_n\to\lambda$ and \eqref{eq:critical-amplitude}, this gives
\[
 \prod_{y_{n,j}\le R}J_0\bigl(t b_{\lambda_n}(y_{n,j})\bigr)
 \longrightarrow
 \prod_{\substack{y\in\mu\\y\le R}}
 J_0\bigl(t b_\lambda(y)\bigr).
\]
For the tails use $|J_0(u)|\le1$ and
\[
 |1-J_0(u)|
 =\left|\mathbb E(1-\cos(u\cos\theta))\right|
 \le\frac{u^2}{4}.
\]
The telescoping product inequality therefore gives, uniformly in $n$,
\[
 \left|1-\prod_{y_{n,j}>R}
 J_0\bigl(t b_{\lambda_n}(y_{n,j})\bigr)\right|
 \le \frac{t^2}{4}\sum_{y_{n,j}>R}b_{\lambda_n}(y_{n,j})^2
 \le C_{\lambda,t}\int_{(R,\infty)}y^{-2}\,d\mu_n(y),
\]
which tends to zero as $R\to\infty$ by
\eqref{eq:critical-tail}.  The identical bound applies to $\mu$ (by
vague approximation and lower semicontinuity).  Thus the full
characteristic functions converge to the product in
\eqref{eq:critical-product}; L\'evy's theorem proves (i).  Applying the
same compact-plus-tail argument to $b^2/2$ proves the variance formula
and its convergence, giving (ii).
...
Finally, for $Z=b\cos\theta$,
$\mathbb EZ^2=b^2/2$, $\mathbb EZ^4=3b^4/8$, and therefore
$\kappa_4(Z)=-3b^4/8$.  Cumulants add for independent sums, and the
sum of fourth powers is finite by the already proved square summability
and $b_\lambda\le4$.  This proves \eqref{eq:critical-kappa4}, which is
...
certificate \texttt{paper/v3/verify\_monotonicity\_arb.py}.  This
...
zero-free analytic leg.  Unlike the density quadrature, this
calculation is entirely in ball arithmetic; a failed interval-Newton
