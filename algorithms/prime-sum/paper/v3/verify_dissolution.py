...
Checks, in order:
  ...
  [3] S4(m) = sum (2a_gamma)^4: zeros+tail evaluation vs the asymptotic
      32 M (log(qM/2pi) - 1), at (q,m) = (4,100), (4,20), (4,8), (3,60).
  [4] High-accuracy deterministic Gil-Pelaez densities delta_q(m) with
      ...
      by [1]), and comparison with the one- and two-term dissolution
      predictions
...
print("    (q,m)      delta            d-1/2      pred1      pred2      rel.err1   rel.err2  tailmodel")
for (q, m) in [(4, 8), (4, 20), (4, 100), (3, 60)]:
    ...
    print(f"    ({q},{m:>3})  {d:.9f}  {dd:.7f}  {pred1:.7f}  {pred2:.7f}  "
          f"{dd/pred1-1:+.2e}  {dd/pred2-1:+.2e}  {smallt:.1e}+{bigt:.1e}={smallt+bigt:.1e}")
